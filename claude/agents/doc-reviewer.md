---
name: doc-reviewer
description: ドキュメント（PRD・機能設計・アーキテクチャ等）の品質をレビューし改善提案を行う。完全性・明確性・一貫性・実装可能性・測定可能性の5観点で評価。/review-docsコマンドまたはドキュメント品質分析が必要な時に使用。
model: sonnet
---
...
| 症状                                     | 原因                       | 対処                                                           |
| ---------------------------------------- | -------------------------- | -------------------------------------------------------------- |
| ドキュメントが見つからない               | パスの指定ミス             | `docs/` 配下のファイル一覧を `Glob('docs/**/*.md')` で確認     |
| レビュー結果が表面的                     | ドキュメント種別の判定ミス | ステップ1でドキュメント種別を正しく特定しているか確認          |
| 他ドキュメントとの整合性チェックが不十分 | 関連ドキュメント未読込     | `docs/` 配下の関連ドキュメントを全て読み込んでからレビュー開始 |
